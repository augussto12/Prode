...
export async function syncKnockoutFixtures(req, res, next) {
  try {
    const result = await syncService.syncKnockoutFixtures(
      req.body.leagueId,
      req.body.season
    );
    res.json({ message: 'Eliminatorias actualizadas', ...result });
  } catch (err) { next(err); }
}
