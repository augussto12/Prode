...
export default function LeagueView() {
  ...
  const loadFixtures = async () => {
    try {
      ...
      setRounds(allRounds);
      // Always save all fixtures for the bracket view
    ...
    <div className="space-y-6">
      ...
      <div className={`glass-card rounded-2xl p-6 relative overflow-hidden ${
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-5 w-full">
          {leagueInfo?.logo ? (
            <img src={leagueInfo.logo} alt={leagueInfo.name} className={`object-contain ${isWorldCup ? 'w-20 h-20' : 'w-16 h-16'}`} />
          ) : (
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex items-center justify-center md:justify-start gap-2 mt-1">
              {isWorldCup ? (
                ...
                <>
                  {countryInfo?.flag && <img src={countryInfo.flag} alt="" className="w-5 h-3 object-contain rounded-sm" />}
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-2xl overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {group.map((row) => {
                        // Use row.description for indicator color if exists
                        ...
                        <tr key={row.team.id} className="border-t border-white/5 hover:bg-white/[0.02]">
                          <td className={`px-4 py-2.5 sticky-col backdrop-blur-md border-r border-l-[3px] border-white/5 ${indicatorColor}`}>
                            <div className="flex items-center gap-2 cursor-pointer group" onClick={() => navigate(`/equipo/${row.team.id}`)}>
                              ...
                              {row.team.logo && <img src={row.team.logo} alt="" className="w-5 h-5 object-contain group-hover:scale-110 transition-transform shrink-0" />}
      ...
      {tab === 'fixtures' && (fixtures.length > 0 || rounds.length > 0) && (
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all border-none cursor-pointer ${
                  fixturesView === 'list' ? 'bg-white/10 text-white' : 'bg-transparent text-white/40 hover:text-white/60'
                }`}
              >
                <List size={16} /> Lista
              </button>
              <button
                onClick={() => {
                  setFixturesView('bracket');
                  // Ensure all fixtures are loaded for bracket
                  if (allFixtures.length === 0) loadFixtures();
                }}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all border-none cursor-pointer ${
                  fixturesView === 'bracket' ? 'bg-white/10 text-white' : 'bg-transparent text-white/40 hover:text-white/60'
                }`}
              >
                <GitBranch size={16} /> Llaves
              </button>
            </div>
          )}
          ...
          {fixturesView === 'list' ? (
            ...
            <TournamentBracket fixtures={allFixtures.length > 0 ? allFixtures : fixtures} />
          )}
        </div>
      )}
                    ...
                    <tr key={player.id} className="border-t border-white/5 hover:bg-white/[0.03] transition-colors cursor-pointer"
                      ...
                      <td className="px-4 py-2 sticky-col backdrop-blur-xl border-r border-white/5 group-hover:bg-white/5 transition-colors">
                        <div className="flex items-center gap-3">
                          ...
                          {player.photo ? (
                            <img src={player.photo} alt="" className="w-8 h-8 rounded-full object-cover bg-white/10" />
                          ) : (
                      ...
                      <td className="px-2 py-3">
                        <div className="flex items-center gap-2">
                          {stats?.team?.logo && <img src={stats.team.logo} alt="" className="w-5 h-5 object-contain" />}
          ...
          {teams.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {teams.map(team => (
                ...
                >
                  {team.logo ? (
                    <img src={team.logo} alt={team.name} loading="lazy" className="w-14 h-14 object-contain group-hover:scale-110 transition-transform" />
                  ) : (
              ...
              return (
                <div key={m.fixture.id}
                  ...
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    {m.teams?.home?.logo && <img src={m.teams.home.logo} alt="" className="w-6 h-6 object-contain" />}
                  ...
                  <div className="flex items-center gap-2 flex-1 min-w-0 justify-end">
                    ...
                    {m.teams?.away?.logo && <img src={m.teams.away.logo} alt="" className="w-6 h-6 object-contain" />}
                  </div>
