...
export default function LeagueView() {
  ...
  const isSynced = competitions.some(
  ...
  const handleEnableProde = async () => {
    ...
    try {
      ...
      addToast({
        type: "success",
        message: "Descargando fixtures y fechas...",
      });
      ...
      addToast({
        type: "success",
        message: "¡Torneo habilitado para el Prode!",
      });
    } catch (err) {
      addToast({
        type: "error",
        message: err.response?.data?.error || "Error al habilitar el torneo",
      });
    } finally {
      setSyncing(false);
    }
  ...
  const loadStandings = async () => {
    try {
      const { data } = await api.get(
        `/explorer/leagues/${id}/standings?season=${season}`,
      );
      setStandings(data);
  ...
  const loadFixtures = async () => {
    try {
      ...
      if (currentRound && !selectedRound) {
        ...
        const { data: currentFixtures } = await api.get(
          `/explorer/leagues/${id}/fixtures?season=${season}&round=${encodeURIComponent(currentRound)}`,
        );
  ...
  const loadFixturesByRound = async (round) => {
    ...
    try {
      const { data } = await api.get(
        `/explorer/leagues/${id}/fixtures?season=${season}&round=${encodeURIComponent(round)}`,
      );
      setFixtures(data);
    } catch (err) {
      console.error(err);
  ...
  const loadScorers = async () => {
    try {
      const { data } = await api.get(
        `/explorer/leagues/${id}/scorers?season=${season}`,
      );
      setScorers(data);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
          ...
          {leagueInfo?.logo ? (
            ...
            >
              <Trophy
                ...
                className={isWorldCup ? "text-amber-400" : "text-white/60"}
              />
            </div>
          )}
              ...
              {isWorldCup ? (
                ...
                <>
                  ...
                  <span className="text-sm text-white/50">
                    {tCountry(countryInfo?.name)}
                  </span>
                  <span className="text-white/40">•</span>
                  <span className="text-sm text-white/50">
                    {leagueInfo?.type === "Cup" ? "Copa" : "Liga"}
                  </span>
                </>
              )}
            </div>
      ...
      {isWorldCup && (
        <div className="flex bg-white/5 p-1 rounded-xl w-full sm:w-fit mb-2">
          <button
            onClick={() => setViewMode("explorer")}
            className={`flex-1 sm:flex-none px-6 py-2 rounded-lg text-sm font-bold transition-all border-none flex items-center justify-center gap-2 cursor-pointer outline-none ${viewMode === "explorer"
      ...
      {viewMode === "prode" && <Competition hideHeader />}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border-none cursor-pointer ${standingsView === "table"
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border-none cursor-pointer ${standingsView === "bracket"
                    ? "bg-white/10 text-white"
                    : "bg-transparent text-white/60 hover:text-white/60"
                    }`}
                >
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin">
                      {standings.league.standings.map((g, i) => (
                        <button
                          ...
                          className={`flex-shrink-0 px-4 py-2 rounded-xl text-sm font-bold transition-all border cursor-pointer ${activeGroupIndex === i
                                ...
                                {group.map((row) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <td className="px-2 py-2.5 text-center text-white/50">
                                        {row.goalsDiff > 0 ? "+" : ""}
                                        {row.goalsDiff}
                                      </td>
                          ...
                          {(() => {
                            ...
                            group.forEach((row) => {
                              ...
                              if (descLower.includes("1/8-finals")) {
                                ...
                                descLower.includes("promotion") ||
                                descLower.includes("champions league") ||
                                descLower.includes("libertadores") ||
                                descLower.includes("playoffs") ||
                                descLower.includes("play-off")
                              ) {
                                indicatorBg = "bg-emerald-500";
                              } else if (descLower.includes("relegation")) {
                                indicatorBg = "bg-red-500";
                              } else if (
                                descLower.includes("qualifiers") ||
                                descLower.includes("europa") ||
                                descLower.includes("sudamericana") ||
                                descLower.includes("knockout")
                              ) {
                              ...
                              if (
                                indicatorBg &&
                                !legends.some((l) => l.text === descText)
                              ) {
                            ...
                            return (
                              <div className="p-4 border-t border-white/5 bg-white/[0.01] flex flex-wrap gap-x-6 gap-y-2">
                                {legends.map((l, idx) => (
                                  ...
                                  >
                                    <div
                                      className={`w-2 h-2 rounded-full ${l.bg}`}
                                    ></div>
                                    {l.text}
                                  </div>
                                ))}
            ...
            <div className="space-y-4">
              ...
              {rounds.length > 0 && (
                <div className="flex items-center gap-3 bg-white/[0.02] border border-white/5 p-2 rounded-xl">
                  <span className="text-xs font-medium text-white/50 pl-2">
                    Fase / Jornada:
                  </span>
                  ...
                  <thead>
                    <tr className="text-white/50 text-[10px] md:text-xs border-b border-white/5 uppercase tracking-wider">
                      <th className="text-left px-3 md:px-4 py-3 font-bold sticky-col backdrop-blur-md">
                        <span className="inline-block w-3 md:w-4 text-white/50">
                          #
                        </span>{" "}
                        Jugador
                      </th>
                        ...
                        >
                          <td className="px-3 md:px-4 py-2 sticky-col backdrop-blur-xl border-r border-white/5 group-hover:bg-white/5 transition-colors">
                            <div className="flex items-center gap-2 md:gap-3">
                              <span className="w-3 md:w-4 text-white/60 text-[10px] md:text-xs font-bold shrink-0">
                                {i + 1}
                              </span>
                              {player.photo ? (
                                <img
                                  ...
                                  width={24}
                                  height={24}
                                  className="w-6 h-6 md:w-8 md:h-8 rounded-full object-cover bg-white/10"
                                  loading="lazy"
                                  decoding="async"
                                ...
                                <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-white/10 flex items-center justify-center text-[10px] md:text-xs font-bold text-white/60">
                                  {player.name?.charAt(0)}
                                </div>
                              )}
                              <div className="min-w-0 pr-2">
                                <div className="text-xs md:text-sm font-bold text-white truncate max-w-[120px] sm:max-w-[180px]">
                                  {player.name}
                                </div>
                                <div className="text-[9px] md:text-[10px] text-white/60 truncate">
                          ...
                          <td className="px-2 py-2 text-center text-white/60 text-xs md:text-sm">
                            {stats?.goals?.assists ?? 0}
                          </td>
                          <td className="px-2 py-2 text-center text-white/60 text-[10px] md:text-xs">
...
function FixturesList({ fixtures }) {
  ...
  [...fixtures]
    ...
    .forEach((m) => {
      const dateKey = new Date(m.fixture.date).toLocaleDateString("es-AR", {
        weekday: "long",
        day: "numeric",
        month: "long",
      });
            ...
            {dayFixtures.map((m) => {
              ...
              return (
                <div
                  ...
                  className={`glass-card rounded-xl p-3 flex items-center cursor-pointer hover:bg-white/[0.06] transition-all ${isLive ? "border border-red-500/20" : ""
                    }`}
                >
                  ...
                  <div className="px-4 text-center min-w-[70px]">
                    {isLive ? (
                      ...
                      <span className="text-xs text-white/60">
                        {new Date(m.fixture.date).toLocaleTimeString("es-AR", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </span>
