...
export default function DreamTeam() {
  ...
  const [team, setTeam] = useState({
    gkId: null,
    def1Id: null, def2Id: null,
    mid1Id: null, mid2Id: null,
    fwd1Id: null, fwd2Id: null
  });
  ...
  const loadData = async () => {
    try {
      ...
      if (teamData) {
        ...
        setTeam({
          gkId: teamData.gkId,
          def1Id: teamData.def1Id, def2Id: teamData.def2Id,
          mid1Id: teamData.mid1Id, mid2Id: teamData.mid2Id,
          fwd1Id: teamData.fwd1Id, fwd2Id: teamData.fwd2Id,
        });
      }
    } catch (err) { console.error('Failed to load dream team data', err); }
  };
  ...
  const handleFormationChange = (newFormation) => {
    ...
    const newLayout = {
      '1-2-1': { GK: ['gkId'], DEF: ['def1Id', 'def2Id'], MID: ['mid2Id'], FWD: ['fwd1Id'] },
      '1-1-2': { GK: ['gkId'], DEF: ['def2Id'], MID: ['mid1Id'], FWD: ['fwd1Id', 'fwd2Id'] },
      '2-1-1': { GK: ['gkId'], DEF: ['def1Id', 'def2Id'], MID: ['mid1Id'], FWD: ['fwd1Id'] }
    }[newFormation];
    ...
    const newTeam = { gkId: null, def1Id: null, def2Id: null, mid1Id: null, mid2Id: null, fwd1Id: null, fwd2Id: null };

    for (const pos of ['GK', 'DEF', 'MID', 'FWD']) {
      const availableSlots = newLayout[pos] || [];
      const playersInPos = currentByPos[pos];
      for (let i = 0; i < Math.min(availableSlots.length, playersInPos.length); i++) {
  ...
  const handleAddPlayer = (player) => {
    ...
    if (!possibleSlots) {
      useToastStore.getState().addToast({ type: 'warning', message: `La formación actual no permite añadir un '${player.position}'.` });
      return;
    }
    ...
    if (foundSlot) {
      ...
      useToastStore.getState().addToast({ type: 'warning', message: `No tenés espacio para más '${player.position}'. Eliminá alguno haciendo click en la cancha.` });
    }
  };
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="glass-card rounded-2xl p-6 relative overflow-hidden">
         ...
         <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              <h1 className="text-3xl font-bold text-white mb-2">Mi Dream Team</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-2">
           <Pitch formation={formation} players={populatedTeam} onRemove={handleRemoveSlot} />
         </div>
                 ...
                 {['ALL', 'GK', 'DEF', 'MID', 'FWD'].map(f => (
                   <button 
                     ...
                     className={`flex-1 py-1.5 rounded-lg text-xs font-bold cursor-pointer transition-colors ${
                       filter === f ? 'bg-indigo-500 text-white border-transparent' : 'bg-transparent text-white/50 border border-white/10 hover:bg-white/5'
                     }`}
                   >
                     {f}
                   </button>
