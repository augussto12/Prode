...
function Fantasy() {
  ...
  return (
    <div className="fantasy-hub page-container fade-in">
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 px-4 relative z-10 w-full mb-10 mt-6">
        {/* MIS LIGAS */}
        <div className="glass-card rounded-2xl p-6 flex flex-col">
          <h2 className="text-2xl font-bold text-white mb-4 flex items-center gap-2 border-b border-white/10 pb-3">
             <Shield className="text-emerald-400" /> Mis Ligas Activas
          </h2>
          {isInitializing ? (
             <div className="text-white/50 text-center py-6">Cargando tus torneos...</div>
          ) : myLeagues.length === 0 ? (
             <div className="flex-1 flex flex-col items-center justify-center text-center p-6 border-2 border-dashed border-white/10 rounded-xl">
                <Users size={48} className="text-white/20 mb-3" />
                <p className="text-white/60 text-sm">Aún no estás participando en la gloria.<br/>Únete a una liga privada o crea la tuya.</p>
             </div>
          ) : (
                      ...
                      <div className="flex justify-between items-center">
                         <div>
                            <h3 className="font-bold text-white flex items-center gap-2">
                              {lg.name}
                              {lg.teamStatus !== 'active' && <span className="bg-red-500 text-xs px-2 py-0.5 rounded text-white font-bold ml-2">BLOQUEADO</span>}
                            </h3>
                            <p className="text-xs text-white/50 mt-1">{lg.realLeagueName} • {lg.maxTeams} Cupos</p>
                         </div>
                         <ChevronRight className={lg.teamStatus === 'active' ? "text-indigo-400" : "text-red-500/50"} />
                      </div>
                   </Link>
                ))}
             </div>
        ...
        <div className="space-y-6">
          <div className="glass-card rounded-2xl p-6 relative overflow-hidden group">
             ...
             <h2 className="text-xl font-bold text-white mb-2">Crear Torneo Prive</h2>
             <p className="text-sm text-white/60 mb-6">Toma las riendas. Elige una liga real, designa reglas y comparte el código secreto con tus amigos (Máx 3 Ligas).</p>
             <button onClick={startWizard} className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold py-3 px-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2">
                <Plus size={20} /> Empezar como Admin
             </button>
          </div>

          <div className="glass-card rounded-2xl p-6 relative overflow-hidden">
             <Lock size={40} className="text-emerald-400 mb-4" />
             <h2 className="text-xl font-bold text-white mb-2">Unirse por Invitación</h2>
             <p className="text-sm text-white/60 mb-4">Si tienes el código de 6 digitos, regístrate y postula tu equipo para las jornadas inminentes.</p>
             <form onSubmit={handleJoin} className="flex gap-2">
                <input 
                   type="text"
                   placeholder="Código F4NT5Y"
                   value={joinCode}
                   onChange={(e) => setJoinCode(e.target.value.toUpperCase())}
                   maxLength={6}
                   className="flex-1 bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-white text-base font-bold tracking-widest uppercase focus:outline-none focus:border-emerald-500 placeholder:normal-case placeholder:tracking-normal"
                />
                <button disabled={loading} type="submit" className="bg-white/10 hover:bg-white/20 text-emerald-400 font-bold py-3 px-6 rounded-xl transition-all whitespace-nowrap">
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
           <div className="bg-[#111827] border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              {/* HEADER */}
              <div className="flex justify-between items-center p-5 border-b border-white/5 bg-white/5">
                 <h3 className="text-lg font-bold text-white flex items-center gap-2"><Trophy className="text-amber-400" size={20}/> Nuevo Torneo</h3>
                 <button onClick={() => setIsCreateModalOpen(false)} className="text-white/50 hover:text-white transition-colors"><X size={24} /></button>
              </div>
