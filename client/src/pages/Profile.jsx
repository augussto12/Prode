...
export default function Profile() {
  ...
  // Efecto de Previsualización en Vivo de los colores
  useEffect(() => {
    const root = document.documentElement;
    root.style.setProperty("--primary-color", form.themePrimary);
    root.style.setProperty("--secondary-color", form.themeSecondary);
    root.style.setProperty("--accent-color", form.themeAccent);
    root.style.setProperty("--bg-start-color", form.themeBgFrom);
    root.style.setProperty("--bg-end-color", form.themeBgTo);
    document.documentElement.style.backgroundColor = form.themeBgTo;
    document.body.style.backgroundColor = "transparent";
  }, [form.themePrimary, form.themeSecondary, form.themeAccent, form.themeBgFrom, form.themeBgTo]);

  // Al desmontar el componente, si se fueron sin guardar, descartamos la previsualización
  useEffect(() => {
    return () => {
        ...
        <form onSubmit={handleSaveProfile} className="space-y-5">
          ...
          <div className="pt-2 border-t border-white/10">
            ...
            <p className="text-[10px] text-white/50 mt-2">
              Estos colores afectarán tu vista a menos que entres a un grupo
              específico.
            </p>
          </div>
          ...
          <button
            ...
            className="w-full sm:w-auto px-8 py-3 bg-indigo-500 hover:bg-indigo-400 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all mt-4 border-none cursor-pointer ms-auto"
          >
            <Save size={18} />{" "}
            {savingProfile ? "Guardando Ajustes..." : "Guardar Perfil"}
          </button>
        </form>
      </div>
      ...
      <div className="glass-card rounded-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        >
          ...
          <button
            ...
            className="w-full sm:w-auto px-8 py-3 bg-amber-500 hover:bg-amber-400 text-stone-900 rounded-xl font-bold flex items-center justify-center gap-2 transition-all mt-4 border-none cursor-pointer ms-auto shadow-[0_0_15px_rgba(245,158,11,0.2)]"
          >
            <Save size={18} />{" "}
