...
export default function TeamView() {
  ...
  const squad = tabData.squad || [];
  const fixtures = tabData.fixtures || [];
  const statistics = tabData.statistics;
  ...
  const loadTeamData = async () => {
    try {
      setLoading(true);
      const [infoRes, coachRes] = await Promise.all([
      ...
      if (coachRes.data && coachRes.data.length > 0) {
        const candidateCoaches = coachRes.data.filter((c) => {
          return c.career.some(
            (career) => career.team.id === Number(id) && career.end === null,
          );
        });
        ...
        if (candidateCoaches.length > 0) {
          activeCoach = candidateCoaches.sort((a, b) => {
            const getLatestStart = (coach) => {
              const careers = coach.career.filter(
                (cr) => cr.team.id === Number(id) && cr.end === null,
              );
              if (!careers.length) return 0;
              return Math.max(
                ...careers.map((cr) => new Date(cr.start).getTime()),
              );
            };
      ...
      // Limpiar tabData del equipo anterior
      setTabData({ squad: null, fixtures: null, transfers: null, statistics: null });
      setActiveTab("squad"); // Reset tab
  ...
  useEffect(() => {
    ...
    const fetchTab = async () => {
      try {
        if (activeTab === "squad" && !tabData.squad) {
          setLoadingTab(true);
          const res = await api.get(`/explorer/teams/${id}/squad`).catch(() => ({ data: [] }));
          setTabData(prev => ({ ...prev, squad: res.data || [] }));
        } else if (activeTab === "fixtures" && !tabData.fixtures) {
          setLoadingTab(true);
          const res = await api.get(`/explorer/teams/${id}/fixtures`).catch(() => ({ data: [] }));
          const sorted = [...(res.data || [])].sort((a, b) => new Date(b.fixture.date) - new Date(a.fixture.date));
          setTabData(prev => ({ ...prev, fixtures: sorted }));
        } else if (activeTab === "transfers" && !tabData.transfers) {
          setLoadingTab(true);
          ...
          if (!currentFixtures) {
            const fRes = await api.get(`/explorer/teams/${id}/fixtures`).catch(() => ({ data: [] }));
            currentFixtures = [...(fRes.data || [])].sort((a, b) => new Date(b.fixture.date) - new Date(a.fixture.date));
            setTabData(prev => ({ ...prev, fixtures: currentFixtures }));
          }
          if (currentFixtures && currentFixtures.length > 0) {
            ...
            const primaryLeagueId = Object.keys(leagueCounts).reduce((a, b) =>
            ...
            setTabData(prev => ({ ...prev, statistics: statsRes.data }));
          } else {
            setTabData(prev => ({ ...prev, statistics: null }));
          }
        }
      } catch (err) {
        console.error(err);
  ...
  const filteredTransfers = useMemo(() => {
    return allTransfers.filter(
      (tr) => transferYear === "" || tr.date.startsWith(transferYear),
    );
  }, [allTransfers, transferYear]);

  const TRANSFERS_PER_PAGE = 10;
  const totalTransferPages = Math.ceil(
    filteredTransfers.length / TRANSFERS_PER_PAGE,
  );
  ...
  return (
    <div className="space-y-6 max-w-8xl mx-auto pb-10">
      <button
        onClick={() => navigate(-1)}
        className="flex items-center gap-2 text-sm text-white/50 hover:text-white transition-all bg-transparent border-none cursor-pointer"
      >
      ...
      <m.div
        ...
        className="glass-card rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-6"
      >
  ...
  onError={(e) => {
        ...
        <div className="flex-1 text-center md:text-left space-y-2">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-sm text-white/60 mt-1">
            ...
            {team.founded && (
              <span className="flex items-center gap-1.5">
                <Calendar size={14} /> Fundado en {team.founded}
              </span>
            )}
          </div>
          {venue && (
            <div className="flex items-center justify-center md:justify-start gap-2 text-sm text-white/60 mt-1">
              <span className="truncate max-w-[200px] md:max-w-md">
                Estadio {venue.name} — {venue.city} (
          ...
          {coach && (
            <div className="mt-4 inline-flex items-center gap-2 bg-white/5 border border-white/10 px-3 py-1.5 rounded-lg">
              ...
              <div className="flex flex-col">
                <span className="text-[9px] uppercase tracking-wider text-white/60 font-bold leading-none">
                  Director Técnico
                </span>
      ...
      {activeTab === "squad" && (
        <m.div
          ...
          className="space-y-6"
        >
          {loadingTab && !tabData.squad ? (
            <div className="w-full h-[400px] bg-white/5 rounded-2xl animate-pulse mt-4" />
          ) : squad.length === 0 ? (
            <div className="text-center py-10 text-white/60 glass-card rounded-2xl">
              No hay información del plantel
            </div>
          ) : (
            <>
              {goalkeepers.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold text-amber-400 uppercase tracking-widest mb-3 border-b border-white/10 pb-2">
                    Arqueros
                  </h3>
              ...
              {defenders.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold text-blue-400 uppercase tracking-widest mb-3 border-b border-white/10 pb-2">
                    Defensores
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              {midfielders.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold text-green-400 uppercase tracking-widest mb-3 border-b border-white/10 pb-2">
                    Mediocampistas
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              {attackers.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold text-red-400 uppercase tracking-widest mb-3 border-b border-white/10 pb-2">
                    Delanteros
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            (() => {
              ...
              const renderFixture = (f) => {
                ...
                if (isLive) {
                  ...
                  resultLetter = f.fixture.status?.elapsed
                ...
                return (
                  <div
                    key={f.fixture.id}
                    onClick={() => navigate(`/partido/${f.fixture.id}`)}
                    className={`glass-card rounded-xl p-3 md:p-4 flex items-center gap-3 cursor-pointer hover:bg-white/5 transition-all ${isLive ? "ring-1 ring-red-500/30" : ""}`}
                  >
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs border shrink-0 ${resultColor}`}
                    >
                      {resultLetter}
                    </div>
  ...
  onError={(e) => {
                    ...
                    <div className="text-lg font-black font-mono shrink-0">
                      {f.score.fulltime.home !== null || isLive ? (
      ...
      {activeTab === "statistics" && (
        ...
        >
          {loadingTab && !tabData.statistics ? (
            ...
            <div className="text-center py-10 text-white/50 glass-card rounded-2xl mt-4">
            ...
            <div className="glass-card rounded-2xl p-5 border border-white/10">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                ...
                <div className="bg-emerald-500/10 rounded-xl p-3 text-center border border-emerald-500/20">
                  ...
                  <div className="text-[10px] text-emerald-400/50 uppercase tracking-wider">
                    Victorias
                  </div>
                </div>
                <div className="bg-red-500/10 rounded-xl p-3 text-center border border-red-500/20">
                  ...
                  <div className="text-[10px] text-red-400/50 uppercase tracking-wider">
                    Derrotas
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                  <div className="text-xs text-white/50 uppercase tracking-wider mb-2 font-bold">
                ...
                <div className="bg-white/5 rounded-xl p-4 border border-white/5 flex flex-col justify-center">
                  ...
                  <div className="text-xl font-black text-blue-300">
                    {statistics.clean_sheet?.total || 0} arcos en cero
                  </div>
                </div>
              </div>
                ...
                <div className="mt-3 bg-white/5 rounded-xl p-4 border border-white/5 text-sm flex justify-between">
                  <div>
                    ...
                    <div className="font-bold text-white">
                      {statistics.biggest.streak?.wins || 0} Partidos
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="font-bold text-white text-emerald-400">
                      {statistics.biggest.wins?.home || "-"} (L) /{" "}
                      {statistics.biggest.wins?.away || "-"} (V)
                    </div>
                  </div>
                </div>
              )}
          ...
          {loadingTab && !tabData.transfers ? (
            ...
            <>
              ...
              {currentTransfers.length === 0 ? (
                <div className="text-center py-10 text-white/60 glass-card rounded-2xl">
                  No hay historial de fichajes recientes publicado para este periodo.
                </div>
              ) : (
            <div className="space-y-3">
              {currentTransfers.map((tr, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="glass-card rounded-xl border border-white/10 overflow-hidden flex flex-col md:flex-row md:items-center"
                  >
                    {/* Left Side: Player */}
                    ...
                    <div className="flex-1 p-4 flex flex-col sm:flex-row items-center gap-4 justify-between">
                      <div className="flex flex-col gap-1 items-center sm:items-start w-full sm:w-auto">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span
                            className={`px-2 py-0.5 rounded uppercase font-black text-[10px] min-w-[50px] text-center ${isIncoming ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}
                          >
              ...
              <button
                onClick={() =>
                  setTransferPage((prev) =>
                    Math.min(totalTransferPages, prev + 1),
                  )
                }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div>
        ...
        {upcoming.length === 0 ? (
          <div className="text-center py-8 text-white/50 text-sm bg-white/[0.02] rounded-xl border border-white/5">
      ...
      <div>
        ...
        {played.length === 0 ? (
          ...
          <>
            <div className="space-y-2">{visiblePlayed.map(renderFixture)}</div>
            {played.length > initialShow && !showAllPlayed && (
              <button
                onClick={() => setShowAllPlayed(true)}
                className="w-full mt-3 py-2 text-xs font-medium text-white/60 bg-white/5 hover:bg-white/10 rounded-xl transition-all border border-white/10 cursor-pointer"
              >
