...
export default function LiveMatch() {
  ...
  const loadMatch = async () => {
    try {
      ...
      const isMatchLive = ["1H", "2H", "HT", "ET", "BT", "P", "LIVE"].includes(
        status,
      );
  ...
  const fetchH2HHistory = async (fix, isSecondLeg, currentId) => {
    try {
      ...
      const matches = (h2hData || []).filter(
        (m) => String(m.fixture.id) !== String(currentId),
      );
  ...
  const fetchLiveOdds = async () => {
    try {
      ...
      console.log("No live odds available");
    }
  };
  ...
  const fetchPreMatchOdds = async () => {
    try {
      ...
      console.log("No pre-match odds available");
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-[1536px] mx-auto pb-10 px-4 xl:px-8">
      ...
      <div
        className={`glass-card rounded-3xl p-6 relative overflow-hidden ${isLive ? "border border-red-500/30" : "border border-white/10"}`}
      >
        {isLive && (
  ...
  onError={(e) => {
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-6 md:gap-12 max-w-4xl mx-auto">
          ...
          <div
            ...
            onClick={() => navigate(`/equipo/${teams?.home?.id}`)}
          >
            {teams?.home?.logo ? (
              <img
                src={teams.home.logo}
                ...
                width={48}
                height={48}
                className="w-12 h-12 md:w-24 md:h-24 mb-1 md:mb-4 object-contain drop-shadow-2xl group-hover:scale-110 transition-transform duration-300"
                loading="lazy"
                decoding="async"
  ...
  onError={(e) => {
          ...
          <div className="px-2 md:px-6 text-center flex flex-col items-center shrink-0">
            <div
              className="text-3xl sm:text-5xl md:text-7xl font-black text-white tracking-tighter whitespace-nowrap"
              style={{ textShadow: "0 4px 20px rgba(0,0,0,0.5)" }}
            >
              {goals?.home ?? 0}{" "}
              <span className="text-white/40 font-normal mx-1 md:mx-3">-</span>{" "}
              {goals?.away ?? 0}
            </div>
            ...
            {firstLeg &&
              (() => {
                ...
                const idaGoalsForCurrentHome =
                  idaHomeId === currentHomeId
                    ? firstLeg.goals?.home
                    : firstLeg.goals?.away;
                const idaGoalsForCurrentAway =
                  idaHomeId === currentHomeId
                    ? firstLeg.goals?.away
                    : firstLeg.goals?.home;

                const aggHome =
                  (goals?.home ?? 0) + (idaGoalsForCurrentHome ?? 0);
                const aggAway =
                  (goals?.away ?? 0) + (idaGoalsForCurrentAway ?? 0);
                ...
                return (
                  <div className="mt-1 md:mt-2 space-y-1">
                    <div className="inline-flex items-center gap-1.5 md:gap-2 px-2 md:px-3 py-0.5 md:py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20">
                      <span className="text-[9px] md:text-[10px] uppercase tracking-widest font-bold text-indigo-400/70">
                        Agregado
                      </span>
                      <span className="text-xs md:text-sm font-black text-indigo-300">
                        {aggHome} - {aggAway}
                      </span>
                    </div>
                    <div className="text-[9px] md:text-[10px] text-white/50 font-medium hidden sm:block">
                      Ida:{" "}
              ...
              {isLive ? (
                <div className="inline-flex items-center justify-center gap-1.5 md:gap-2 px-2 md:px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 whitespace-nowrap">
                  ...
                  {match?.status?.short === "HT" && (
                    <span className="text-[10px] md:text-xs text-red-500/70 ml-1">
                      (MT)
                    </span>
                  )}
                </div>
          ...
          <div
            className="flex-1 flex flex-col items-center text-center cursor-pointer group"
            onClick={() => navigate(`/equipo/${teams?.away?.id}`)}
          >
            {teams?.away?.logo ? (
              <img
                src={teams.away.logo}
            ...
            <div className="flex-1 space-y-1.5 flex flex-col items-end">
              {goalEvents
                ...
                .map((e, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 text-sm text-white/60 flex-row-reverse"
                  >
                    <span className="text-[10px]">⚽</span>
                    <span className="font-medium text-white/90">
                      {e.player.name}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        >
          ...
          {statistics.length >= 2 && (
            ...
            >
              <h3 className="text-sm font-bold text-white uppercase tracking-widest mb-6 flex items-center justify-center gap-2">
                <span className="text-white/50">📊</span> Estadísticas
              </h3>
              <div className="space-y-4">
                  ...
                  return (
                    <div key={stat.type}>
                      <div className="flex items-center justify-between text-xs text-white/50 mb-1.5">
                        <span
                          className={`font-bold w-10 ${homeNum > awayNum ? "text-emerald-400" : homeNum < awayNum ? "text-white/50" : "text-white"}`}
                        >
                          {homeStat?.value ?? 0}
                        </span>
                        <span className="text-[10px] uppercase font-semibold tracking-wider text-white/50 text-center flex-1">
                          {tStat(stat.type)}
                        </span>
                        <span
                          className={`font-bold w-10 text-right ${awayNum > homeNum ? "text-emerald-400" : awayNum < homeNum ? "text-white/50" : "text-white"}`}
                        >
                          {awayStat?.value ?? 0}
                        </span>
                      </div>
                    ...
                    .map((inj, idx) => (
                      ...
                      >
                        <div className="w-5 h-5 md:w-8 md:h-8 rounded-full overflow-hidden bg-white/5 shrink-0 border border-white/10">
                          <img                             src={`https://media.api-sports.io/football/players/${inj.player.id}.png`}
                            alt=""
                            loading="lazy"
                            width={20}
                            height={20}
  ...
  onError={(e) => {
                          ...
                          />
                          <div
                            className="w-full h-full items-center justify-center text-[9px] md:text-[10px] font-bold text-white/50"
                            style={{ display: "none" }}
                          >
                            {inj.player.name[0]}
                          </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-[10px] md:text-xs font-bold text-white truncate leading-tight">
                            {inj.player.name}
                          </div>
                          <div
                            className={`text-[8px] md:text-[9px] uppercase tracking-wider font-bold truncate ${inj.player.type === "Missing Fixture" ? "text-red-400" : "text-amber-400"}`}
                          >
                            {tInjury(
                              inj.player.reason?.split("(")[0]?.trim(),
                            ) ||
                              tInjury(inj.player.type) ||
                              "En duda"}
                          </div>
                        </div>
                      </div>
                ...
                <div className="w-1/2 pl-2 md:pl-4 space-y-3 md:space-y-4">
                  ...
                  {injuries
                    ...
                    .map((inj, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-1.5 md:gap-2 flex-row-reverse text-right"
                      >
                        <div className="w-5 h-5 md:w-8 md:h-8 rounded-full overflow-hidden bg-white/5 shrink-0 border border-white/10">
                          <img                             src={`https://media.api-sports.io/football/players/${inj.player.id}.png`}
                            alt=""
  ...
  onError={(e) => {
          ...
          {events.length > 0 && (
            ...
            >
              <h3 className="text-sm font-bold text-white uppercase tracking-widest mb-6 flex items-center justify-center gap-2">
                <span className="text-white/50">⚡</span> Cronología
              </h3>
              <div className="relative flex flex-col space-y-3 py-2">
                ...
                {events.map((e, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`flex w-full ${isHome ? "justify-start" : "justify-end"} relative`}
                    >
                      {/* Central Dot */}
                      <div
                        className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 rounded-full flex items-center justify-center text-[10px] border border-white/20 bg-[#0f1115] z-10 shadow-lg ${bg}`}
                      >
                        {icon}
                      </div>
                      ...
                      {/* Content Box */}
                      <div
                        className={`w-[46%] flex flex-col ${isHome ? "items-end text-right" : "items-start text-left"}`}
                      >
                        <div
                          className={`bg-white/[0.02] border ${border} rounded-xl p-2 md:p-3 inline-block max-w-full`}
                        >
                          <div
                            className={`flex items-center gap-1.5 mb-1 ${isHome ? "justify-end" : "justify-start"}`}
                          >
                            {isHome && e.type === "Goal" && (
                              <span className="text-[10px] uppercase font-bold text-emerald-500/50 hidden md:block">
                                GOL
                              </span>
                            )}
                            {isHome &&
                              (e.detail === "Red Card" ||
                                e.detail === "Second Yellow card") && (
                                <span className="text-[10px] uppercase font-bold text-red-500/50 hidden md:block">
                                  ROJA
                                </span>
                              )}
                          ...
                          <div className="flex flex-col">
                            <span
                              className={`text-xs md:text-sm font-bold ${textColor}`}
                            >
                              {e.player?.name}
                            </span>
                            ...
                            {e.type === "Goal" && e.assist?.name && (
                              <span className="text-xs text-white/60 mt-0.5 whitespace-nowrap overflow-hidden text-ellipsis">
...
const LineupCard = memo(function LineupCard({ lineup, align, navigate }) {
  ...
  const sortedRows = Object.keys(gridRows).sort(
  ...
  return (
    ...
    >
      <div
        className={`flex items-center gap-3 mb-5 pb-4 border-b border-white/5 ${align === "right" ? "flex-row-reverse" : ""}`}
      >
        {lineup.team?.logo && (
          <img
  ...
  onError={(e) => {
        ...
        <div className={`flex-1 ${align === "right" ? "text-right" : ""}`}>
          ...
          {lineup.formation && (
            <div className="text-[10px] text-amber-400 font-mono tracking-widest bg-amber-400/10 px-2 py-0.5 rounded-full inline-block mt-1">
              {lineup.formation}
            </div>
          )}
        </div>
      ...
      {lineup.coach && (
        <div
          className={`flex items-center gap-2 mb-5 px-1 bg-white/[0.02] rounded-xl p-2 border border-white/5 ${align === "right" ? "flex-row-reverse" : ""}`}
        >
          {lineup.coach.photo ? (
            <img
  ...
  onError={(e) => {
            ...
            <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-[10px] font-bold text-white/50 border border-white/5">
              DT
            </div>
          )}
      ...
      <div
        className={`text-xs text-white/50 uppercase tracking-widest mb-3 font-semibold px-2 ${align === "right" ? "text-right" : ""}`}
      >
        Titulares
      </div>
      ...
      {hasGrid ? (
        <div
          className="relative w-full max-w-[400px] mx-auto aspect-[4/5] bg-emerald-700/80 rounded-2xl overflow-hidden mb-6 border-2 border-white/20 shadow-[inset_0_0_50px_rgba(0,0,0,0.5)] flex flex-col justify-between py-6 group/pitch"
          style={{
        ...
        >
          ...
          {sortedRows.map((rowNum) => (
            <div
              key={rowNum}
              className="relative z-10 flex justify-around w-full px-4"
            >
              {gridRows[rowNum].map((p, index) => {
                ...
                return (
                  <m.div
                    ...
                    onClick={() =>
                      p.player?.id && navigate(`/jugador/${p.player.id}`)
                    }
                  >
                    <div className="relative">
                      <div
                        className={`w-9 h-9 rounded-full flex items-center justify-center shadow-lg border-2 z-10 relative transition-transform overflow-hidden group-hover/player:scale-110 group-hover/player:-translate-y-1 ${align === "right" ? "border-indigo-400 bg-indigo-400/20" : "border-white bg-white/20"}`}
                      >
                        <img                           src={`https://media.api-sports.io/football/players/${p.player.id}.png`}
                          alt={shortName}
                          loading="lazy"
  ...
  onError={(e) => {
                        ...
                        />
                        <div
                          className="absolute inset-0 items-center justify-center text-xs font-bold text-black bg-white"
                          style={{ display: "none" }}
                        >
                          {p.player.number || "-"}
                        </div>
        ...
        <div className="space-y-1.5 mb-6">
          {(lineup.startXI || []).map((p, pi) => (
            ...
            >
              ...
              <span
                className={`text-xs font-medium text-white/90 truncate flex-1 ${align === "right" ? "text-right" : ""}`}
              >
                {p.player?.name}
              </span>
              <span className="text-[9px] text-white/40 font-bold uppercase tracking-widest w-8 text-center">
                {p.player?.pos}
              </span>
            </div>
          ))}
      ...
      {(lineup.substitutes || []).length > 0 && (
        <>
          <div
            className={`text-xs text-white/50 uppercase tracking-widest mt-4 mb-3 font-semibold px-2 ${align === "right" ? "text-right" : ""}`}
          >
            Suplentes
          </div>
          <div className="space-y-1">
            {(lineup.substitutes || []).map((p, pi) => (
              ...
              >
                <span className="w-7 text-center text-xs font-mono text-white/60 shrink-0">
                  {p.player?.number}
                </span>
                <span
                  className={`text-xs text-white/70 truncate flex-1 ${align === "right" ? "text-right" : ""}`}
                >
                  {p.player?.name}
                </span>
                <span className="text-[10px] text-white/50 font-bold uppercase tracking-widest w-8 text-center">
                  {p.player?.pos}
                </span>
              </div>
            ))}
