...
export default function AggregateScore({ homeId, awayId, leagueId, round, homeGoalsToday, awayGoalsToday }) {
  ...
  useEffect(() => {
    ...
    const isKnockout = /quarter|semi|final|round of|8th|16th|leg/i.test(round || '');
  ...
  const fetchFirstLeg = async () => {
    try {
      ...
      const firstLegMatch = matches.find(m => {
        ...
        const isSameRoundDesc = r === round || r.includes(round) || /1st\s*Leg/i.test(r);
        
        // Chequear que los equipos estén invertidos (local de hoy era visitante de ayer)
        const isInverted = m.teams?.home?.id === awayId && m.teams?.away?.id === homeId;
  ...
  return (
    <div className="text-center mt-1" title={`Global: ${aggregate.totalHome} - ${aggregate.totalAway}`}>
