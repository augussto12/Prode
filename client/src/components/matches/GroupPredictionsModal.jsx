...
export default function GroupPredictionsModal({
  ...
  const loadPredictions = async () => {
    ...
    try {
      const res = await api.get(
        `/groups/${groupId}/matches/${match.externalId || match.id}/predictions`,
      );
      setData(res.data);
    } catch (err) {
          ...
          <div className="p-4 border-b border-white/10 bg-white/5 flex items-center justify-between shrink-0">
            <div>
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <Users size={16} className="text-indigo-400" /> Predicciones del
                Grupo
              </h3>
              {match && (
  ...
  onError={(e) => (e.target.src = "/placeholder-team.svg")
                  ...
                  typeof match.homeGoals === "number" ? (
                    ...
                    <span className="text-white/50 text-[10px]">vs</span>
                  )}
                  <span className="flex items-center gap-1 font-medium text-white/80">
                    {match.awayTeamLogo && (
  ...
  onError={(e) => (e.target.src = "/placeholder-team.svg")
            ...
            <button
              onClick={onClose}
              className="p-1.5 hover:bg-white/10 rounded-full text-white/50 hover:text-white transition-colors cursor-pointer border-none bg-transparent"
            >
              <X size={18} />
            </button>
          </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <span
                            className={`text-[9px] sm:text-[10px] font-bold ${badgeColor}`}
                          >
                            {badge}
                          </span>
