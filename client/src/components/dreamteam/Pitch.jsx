...
export default function Pitch({ formation, players, onRemove }) {
  ...
  const renderSlot = (slotKey, role) => {
    ...
    return (
      <div 
        ...
        style={{ top: layout[slotKey].top, left: layout[slotKey].left, zIndex: 10 }}
        onClick={() => {
           if (pInfo) { onRemove(slotKey); }
        }}
      >
        {pInfo ? (
  ...
  return (
    <div className="w-full max-w-[500px] aspect-[2/3] mx-auto rounded-3xl overflow-hidden shadow-2xl relative border-8 border-green-900/50 bg-[#2d5a27]"
         style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.05) 1px, transparent 1px)', backgroundSize: '100% 10%' }}>
