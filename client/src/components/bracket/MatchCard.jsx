...
export default function MatchCard({ matchup, index = 0, onSelect }) {
  ...
  const Row = ({ team, goals, isWinner, isLoser }) => (
    <div className={`flex items-center gap-1.5 px-2 py-[5px] transition-all ${isWinner ? 'bg-emerald-500/[0.06]' : ''}`}>
      {hasLogo(team) ? (
        <img src={team.logo} alt="" className="w-4 h-4 object-contain shrink-0" loading="lazy" onError={(e) => { e.target.src = '/placeholder-team.svg'; }} />
      ) : (
      ...
      >{team.name}</span>
      <span className={`text-[11px] font-bold font-mono min-w-[14px] text-right shrink-0 ${
        isWinner ? 'text-emerald-300' : isLoser ? 'text-slate-500' : 'text-white/50'
      }`}>{isPlayed ? (goals ?? '-') : '-'}</span>
      {isWinner && <span className="text-emerald-400 text-[8px] shrink-0">✓</span>}
    </div>
  );
  ...
  return (
    <m.div
      ...
      transition={{ delay: Math.min(index * 0.03, 0.6), duration: 0.3, ease: 'easeOut' }}
      ...
      className={`bg-slate-900/90 border border-slate-700/70 rounded-lg shadow-lg overflow-hidden w-full transition-colors ${
        isPlayed ? 'cursor-pointer hover:border-slate-600/80' : 'cursor-default opacity-70'
      }`}
    ...
    >
      ...
      <div className="flex items-center justify-between px-2 py-[2px] bg-slate-800/30">
        ...
        <span className="text-[8px] text-slate-500 font-medium">
          {isLive && liveMinute ? '' : isFinished ? '' : formatDate(matchDate) || ''}
        </span>
        <span className={`text-[8px] font-bold uppercase tracking-wider flex items-center gap-0.5 ${
      ...
      {isAggregate && legs.length > 1 && isPlayed && (
        <div className="px-2 py-[2px] bg-slate-800/15 border-t border-slate-700/20 flex gap-1.5 justify-center">
          {legs.map((leg, li) => {
            const legPlayed = ['FT', 'AET', 'PEN'].includes(leg.fixture.status?.short);
            return (
              <span key={leg.fixture.id} className="text-[8px] text-slate-500">
                {li === 0 ? 'I' : 'V'}: {legPlayed ? `${leg.goals?.home ?? '?'}-${leg.goals?.away ?? '?'}` : '-'}
              </span>
            );
          })}
        </div>
