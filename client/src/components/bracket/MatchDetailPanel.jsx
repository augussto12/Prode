...
export default function MatchDetailPanel({ matchup, onClose }) {
  ...
  const { teamA, teamB, aggA, aggB, legs, winnerId, isFinished, isLive, isAggregate } = matchup || {};
  ...
  useEffect(() => {
    if (!matchup) { setStatsMap({}); setActiveLegIdx(-1); }
  }, [matchup]);
            ...
            <div className="p-5 pt-8 space-y-5">
              ...
              <div className="text-center">
                <span className={`text-[10px] font-bold uppercase tracking-[0.2em] px-3 py-1 rounded-full ${
                  isLive ? 'bg-red-500/20 text-red-400' : isFinished ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700/50 text-slate-400'
                }`}>
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                >
                  <img src={teamA.logo} alt="" className="w-14 h-14 object-contain group-hover:scale-110 transition-transform" loading="lazy" decoding="async" onError={(e) => { e.target.src = '/placeholder-team.svg'; }} />
                  <span className={`text-xs font-bold text-center ${winnerId === teamA.id ? 'text-emerald-300' : 'text-white'}`}>
                    {teamA.name}
                  </span>
                </div>
                ...
                <div className="text-center px-3">
                  <div className="text-3xl font-black text-white tracking-wider font-mono">
                    {aggA ?? '-'} <span className="text-slate-600">:</span> {aggB ?? '-'}
                  </div>
                  {isAggregate && (
                    <span className="text-[9px] text-slate-500 uppercase tracking-wider">Global</span>
                  )}
                </div>
                ...
                >
                  <img src={teamB.logo} alt="" className="w-14 h-14 object-contain group-hover:scale-110 transition-transform" loading="lazy" decoding="async" onError={(e) => { e.target.src = '/placeholder-team.svg'; }} />
                  <span className={`text-xs font-bold text-center ${winnerId === teamB.id ? 'text-emerald-300' : 'text-white'}`}>
                    {teamB.name}
                  </span>
                </div>
              </div>
                ...
                <div className="flex justify-center gap-2">
                  {legs.map((leg, li) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-slate-500 mr-1">{li === 0 ? 'Ida' : 'Vuelta'}</span>
                        {isPlayed ? `${leg.goals?.home ?? '?'} - ${leg.goals?.away ?? '?'}` : '— : —'}
                      </button>
                    );
                  })}
                </div>
              ...
              {isLoading ? (
                ...
                <div className="space-y-0.5">
                  <h3 className="text-[10px] text-slate-500 uppercase tracking-wider font-bold mb-2 text-center">
                    Estadísticas — {activeLegIdx === 0 ? 'Ida' : legs?.length > 1 ? 'Vuelta' : 'Partido'}
                  </h3>
