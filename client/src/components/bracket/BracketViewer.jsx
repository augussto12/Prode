...
function computeLayout(columns, containerWidth = 0) {
  if (!columns?.length)
    return { nodes: [], connectors: [], width: 0, height: 0, phaseHeaders: [] };
  const nodes = [],
    connectors = [];
  ...
  const gap =
    N > 1
      ? Math.max(MIN_COL_GAP, (effW - PADDING_X * 2 - N * CARD_W) / (N - 1))
      : 0;
  const cx = (ci) => PADDING_X + ci * (CARD_W + gap);
  const pos = [];
  pos[0] = columns[0].matchups.map((_, mi) => ({
    x: cx(0),
    y: PADDING_TOP + mi * (CARD_H + ROW_GAP),
  }));
  for (let ci = 1; ci < N; ci++) {
    const prev = columns[ci - 1];
    pos[ci] = columns[ci].matchups.map((m, mi) => {
      const ff = prev.matchups
        .map((pm, pmi) => (pm._nextMatchIndex === mi ? pmi : -1))
        .filter((i) => i >= 0);
      let y;
      if (ff.length >= 2 && pos[ci - 1][ff[0]] && pos[ci - 1][ff[1]])
        y = (pos[ci - 1][ff[0]].y + pos[ci - 1][ff[1]].y) / 2;
  ...
  columns.forEach((col, ci) =>
    col.matchups.forEach((m, mi) =>
      nodes.push({ matchup: m, x: pos[ci][mi].x, y: pos[ci][mi].y }),
    ),
  );
  for (let ci = 0; ci < N - 1; ci++) {
    columns[ci].matchups.forEach((m, mi) => {
      ...
      const f = pos[ci][mi],
        t = pos[ci + 1][nmi];
      connectors.push({
        path: `M ${f.x + CARD_W} ${f.y + CARD_H / 2} H ${(f.x + CARD_W + t.x) / 2} V ${t.y + CARD_H / 2} H ${t.x}`,
        isWinnerPath: m.isFinished && m.winnerId != null,
      });
    });
  }
  ...
  return {
    ...
    connectors,
    width: effW,
    height: Math.max(...allY, 0) + CARD_H + PADDING_BOTTOM,
    phaseHeaders: columns.map((c, ci) => ({
      label: tRound(c.phase),
      x: cx(ci),
    })),
  };
}
...
function computeMirroredLayout(columns, containerWidth = 0) {
  ...
  const gap =
    TV > 1
      ? Math.max(MIN_COL_GAP, (effW - PADDING_X * 2 - TV * CARD_W) / (TV - 1))
      : 0;
  ...
  const LP = [],
    RP = [];
  const leftCount = LM[0].length,
    maxFirst = Math.max(leftCount, RM[0].length);
  ...
  for (let ri = 1; ri < NR; ri++) {
    LP[ri] = LM[ri].map((m, mi) => {
      const ff = LM[ri - 1]
        ...
        .filter((i) => i >= 0);
      let y;
      if (ff.length >= 2 && LP[ri - 1][ff[0]] && LP[ri - 1][ff[1]])
        y = (LP[ri - 1][ff[0]].y + LP[ri - 1][ff[1]].y) / 2;
  ...
  for (let ri = 0; ri < NR; ri++)
    LM[ri].forEach((m, mi) =>
      nodes.push({ matchup: m, x: LP[ri][mi].x, y: LP[ri][mi].y }),
    );
  for (let ri = 0; ri < NR - 1; ri++) {
    LM[ri].forEach((m, mi) => {
      if (
        m._nextMatchIndex == null ||
        m._nextMatchIndex < 0 ||
        !LP[ri + 1]?.[m._nextMatchIndex]
      )
        return;
      const f = LP[ri][mi],
        t = LP[ri + 1][m._nextMatchIndex];
      connectors.push({
        path: `M ${f.x + CARD_W} ${f.y + CARD_H / 2} H ${(f.x + CARD_W + t.x) / 2} V ${t.y + CARD_H / 2} H ${t.x}`,
        isWinnerPath: m.isFinished && m.winnerId != null,
      });
  ...
  const rightCount = RM[0].length,
    rightTotalH = rightCount * (CARD_H + ROW_GAP) - ROW_GAP;
  const rightOffY = PADDING_TOP + (totalH - rightTotalH) / 2;
  RP[0] = RM[0].map((_, mi) => ({
    x: cx(rvc(0)),
    y: rightOffY + mi * (CARD_H + ROW_GAP),
  }));
  for (let ri = 1; ri < NR; ri++) {
    RP[ri] = RM[ri].map((m, mi) => {
      const ff = rnl[ri - 1]
        .map((nxt, pmi) => (nxt === mi ? pmi : -1))
        .filter((i) => i >= 0);
      let y;
      if (ff.length >= 2 && RP[ri - 1][ff[0]] && RP[ri - 1][ff[1]])
        y = (RP[ri - 1][ff[0]].y + RP[ri - 1][ff[1]].y) / 2;
  ...
  for (let ri = 0; ri < NR; ri++)
    RM[ri].forEach((m, mi) =>
      nodes.push({ matchup: m, x: RP[ri][mi].x, y: RP[ri][mi].y }),
    );
  for (let ri = 0; ri < NR - 1; ri++) {
    RM[ri].forEach((m, mi) => {
      ...
      const f = RP[ri][mi],
        t = RP[ri + 1][toMi];
      connectors.push({
        path: `M ${f.x} ${f.y + CARD_H / 2} H ${(f.x + t.x + CARD_W) / 2} V ${t.y + CARD_H / 2} H ${t.x + CARD_W}`,
        isWinnerPath: m.isFinished && m.winnerId != null,
      });
    });
  }
  if (finalM && LP[NR - 1]?.[0] && RP[NR - 1]?.[0]) {
    ...
    connectors.push({
      path: `M ${ls.x + CARD_W} ${ls.y + CARD_H / 2} H ${(ls.x + CARD_W + fx) / 2} V ${fy2} H ${fx}`,
      isWinnerPath:
        LM[NR - 1][0]?.isFinished && LM[NR - 1][0]?.winnerId != null,
    });
    connectors.push({
      path: `M ${rs.x} ${rs.y + CARD_H / 2} H ${(rs.x + fx + CARD_W) / 2} V ${fy2} H ${fx + CARD_W}`,
      isWinnerPath:
        RM[NR - 1][0]?.isFinished && RM[NR - 1][0]?.winnerId != null,
    });
  }
  const allY = nodes.map((n) => n.y);
  const ph = [];
  for (let ri = 0; ri < NR; ri++)
    ph.push({ label: tRound(rounds[ri].phase), x: cx(lvc(ri)) });
  if (hasFinal) ph.push({ label: "Final", x: cx(fvc) });
  for (let ri = 0; ri < NR; ri++)
    ph.push({ label: tRound(rounds[ri].phase), x: cx(rvc(ri)) });
  return {
    nodes,
    connectors,
    width: effW,
...
function computeVerticalLayout(columns, containerWidth) {
  ...
  const maxCards = Math.max(
    ...rounds.map((c) => Math.ceil(c.matchups.length / 2)),
  );
  const cardW = Math.min(
    Math.floor(
      (containerWidth - M_PAD * 2 - Math.max(0, maxCards - 1) * M_CARD_GAP) /
        Math.max(maxCards, 1),
    ),
    170,
  );
    ...
    for (let ri = 0; ri < halfMatchups.length; ri++) {
      ...
      if (ri < halfMatchups.length - 1) {
        ...
        Object.entries(groups).forEach(([nmiStr, parents]) => {
          ...
          const toY = nextCardY;
          const anyWinner = parents.some(
            (p) => p.m.isFinished && p.m.winnerId != null,
          );
          ...
          parents.forEach(({ mi }) => {
            ...
            connectors.push({
              path: `M ${fromX} ${currentY} V ${midY}`,
              isWinnerPath: anyWinner,
            });
          });
          // Horizontal bar connecting all parents at midY
          if (parents.length >= 2) {
            const xs = parents
              .map((p) => roundPos[p.mi].x + cardW / 2)
              .sort((a, b) => a - b);
            connectors.push({
              path: `M ${xs[0]} ${midY} H ${xs[xs.length - 1]}`,
              isWinnerPath: anyWinner,
            });
          }
          ...
          const centerX =
            parents.length >= 2
              ? (Math.min(...parents.map((p) => roundPos[p.mi].x + cardW / 2)) +
                  Math.max(
                    ...parents.map((p) => roundPos[p.mi].x + cardW / 2),
                  )) /
                2
              : roundPos[parents[0].mi].x + cardW / 2;
          // Horizontal from center to child X, then vertical down
          connectors.push({
            path: `M ${centerX} ${midY} H ${toX} V ${toY}`,
            isWinnerPath: anyWinner,
          });
        });
  ...
  if (finalM) {
    ...
    if (lastLeftPos) {
      ...
      connectors.push({
        path: `M ${fromX} ${fromY} V ${midY} H ${toX} V ${toY}`,
        isWinnerPath:
          LM[NR - 1][0]?.isFinished && LM[NR - 1][0]?.winnerId != null,
      });
    }
    ...
    if (firstRightSF) {
      ...
      const rightSFRowW =
        rightSFCount * cardW + (rightSFCount - 1) * M_CARD_GAP;
  ...
  for (let ri = 0; ri < reversedRM.length; ri++) {
    ...
    phaseHeaders.push({
      label: tRound(reversedPhases[ri]),
      y: currentY,
      count,
    });
    currentY += M_HEADER_H;
    ...
    if (ri < reversedRM.length - 1) {
      ...
      if (origRnl) {
        ...
        Object.entries(groups).forEach(([parentMiStr, children]) => {
          ...
          const anyWinner = children.some(
            (c) => c.m.isFinished && c.m.winnerId != null,
          );
          ...
          const childXs = children
            .map((c) => nextStartX + c.mi * (cardW + M_CARD_GAP) + cardW / 2)
            .sort((a, b) => a - b);
          if (childXs.length >= 2) {
            connectors.push({
              path: `M ${childXs[0]} ${midY} H ${childXs[childXs.length - 1]}`,
              isWinnerPath: anyWinner,
            });
          }
          ...
          connectors.push({
            path: `M ${fromX} ${midY} H ${childCenter}`,
            isWinnerPath: anyWinner,
          });
  ...
  for (let ci = 0; ci < columns.length; ci++) {
    ...
    if (ci < columns.length - 1) {
      ...
      col.matchups.forEach((m, mi) => {
        ...
        const midY = currentY + M_ROUND_GAP / 2;
        connectors.push({
          path: `M ${fromX} ${currentY} V ${midY} H ${toX} V ${currentY + M_ROUND_GAP + M_HEADER_H}`,
          isWinnerPath: m.isFinished && m.winnerId != null,
        });
      });
      currentY += M_ROUND_GAP;
...
function MobileVerticalCard({ matchup, index, cardW, onSelect }) {
  ...
  const Row = ({ team, goals, isWinner, isLoser }) => (
    <div
      className={`flex items-center gap-1 px-1.5 py-[3px] ${isWinner ? "bg-emerald-500/[0.06]" : ""}`}
    >
      {hasLogo(team) ? (
        <img           src={team.logo}
          alt=""
  ...
  onError={(e) => {
      ...
      <span
        className={`text-[10px] font-bold font-mono min-w-[10px] text-right shrink-0 ${
          isWinner
            ? "text-emerald-300"
            : isLoser
              ? "text-slate-500"
      ...
      {isWinner && (
        <span className="text-emerald-400 text-[7px] shrink-0">✓</span>
      )}
    </div>
  );
  ...
  return (
    <m.div
      ...
      className={`bg-slate-900/90 border border-slate-700/70 rounded-md overflow-hidden transition-colors ${
        isPlayed
          ? "cursor-pointer active:bg-slate-800/90"
...
function PlayoffsList({ matchups, onSelect, isMobile }) {
  ...
  return (
    <div
      className={`grid gap-2 ${isMobile ? "grid-cols-1" : "grid-cols-2 sm:grid-cols-3 lg:grid-cols-4"}`}
    >
      {matchups.map((m, i) =>
        isMobile ? (
          <MobileVerticalCard
...
function computeVerticalLayoutFixed(columns, containerWidth) {
  ...
  LP_pos[0] = LM[0].map((_, mi) => ({
    x: startX_L + mi * (cardW + M_CARD_GAP),
  }));

  for (let ri = 1; ri < NR; ri++) {
    LP_pos[ri] = LM[ri].map((m, mi) => {
      const parentIndices = LM[ri - 1]
        .map((pm, pmi) => (pm._nextMatchIndex === mi ? pmi : -1))
        .filter((i) => i >= 0);
      let x = startX_L + mi * (cardW + M_CARD_GAP);
      if (parentIndices.length >= 2)
        x =
          (LP_pos[ri - 1][parentIndices[0]].x +
            LP_pos[ri - 1][parentIndices[1]].x) /
          2;
      else if (parentIndices.length === 1)
        x = LP_pos[ri - 1][parentIndices[0]].x;
      return { x };
    });
  }
  ...
  RP_pos[0] = RM[0].map((_, mi) => ({
    x: startX_R + mi * (cardW + M_CARD_GAP),
  }));

  for (let ri = 1; ri < NR; ri++) {
    RP_pos[ri] = RM[ri].map((m, mi) => {
      const parentIndices = rnl[ri - 1]
        .map((nxt, pmi) => (nxt === mi ? pmi : -1))
        .filter((i) => i >= 0);
      let x = startX_R + mi * (cardW + M_CARD_GAP);
      if (parentIndices.length >= 2)
        x =
          (RP_pos[ri - 1][parentIndices[0]].x +
            RP_pos[ri - 1][parentIndices[1]].x) /
          2;
      else if (parentIndices.length === 1)
        x = RP_pos[ri - 1][parentIndices[0]].x;
      return { x };
    });
  }
  ...
  for (let ri = 0; ri < NR; ri++) {
    ...
    if (ri < NR - 1) {
      ...
      LM[ri + 1].forEach((_, targetMi) => {
        const sourceIndices = LM[ri]
          .map((pm, pmi) => (pm._nextMatchIndex === targetMi ? pmi : -1))
          .filter((i) => i >= 0);
        if (sourceIndices.length === 0) return;

        const toX = LP_pos[ri + 1][targetMi].x + cardW / 2;
        const anyWinner = sourceIndices.some(
          (pmi) => LM[ri][pmi].isFinished && LM[ri][pmi].winnerId != null,
        );

        if (sourceIndices.length >= 2) {
          const xs = sourceIndices
            .map((pmi) => LP_pos[ri][pmi].x + cardW / 2)
            .sort((a, b) => a - b);
          sourceIndices.forEach((pmi) =>
            connectors.push({
              path: `M ${LP_pos[ri][pmi].x + cardW / 2} ${roundBottomY} V ${midY}`,
              isWinnerPath: anyWinner,
            }),
          );
          connectors.push({
          ...
          connectors.push({
            path: `M ${toX} ${midY} V ${nextTopY}`,
            isWinnerPath: anyWinner,
          });
        } else {
          const fromX = LP_pos[ri][sourceIndices[0]].x + cardW / 2;
          connectors.push({
            path: `M ${fromX} ${roundBottomY} V ${nextTopY}`,
            isWinnerPath: anyWinner,
          });
        }
      });
  ...
  for (let ri = NR - 1; ri >= 0; ri--) {
    ...
    if (ri > 0) {
      ...
      RM[ri].forEach((_, sourceMi) => {
        const targetIndices = RM[ri - 1]
          .map((cm, cmi) => (rnl[ri - 1][cmi] === sourceMi ? cmi : -1))
          .filter((i) => i >= 0);
        if (targetIndices.length === 0) return;

        const fromX = RP_pos[ri][sourceMi].x + cardW / 2;
        const anyWinner = targetIndices.some(
          (cmi) =>
            RM[ri - 1][cmi].isFinished && RM[ri - 1][cmi].winnerId != null,
        );

        if (targetIndices.length >= 2) {
          const xs = targetIndices
            .map((cmi) => RP_pos[ri - 1][cmi].x + cardW / 2)
            .sort((a, b) => a - b);
          connectors.push({
            path: `M ${fromX} ${roundBottomY} V ${midY}`,
            isWinnerPath: anyWinner,
          });
          connectors.push({
            path: `M ${xs[0]} ${midY} H ${xs[xs.length - 1]}`,
          ...
          targetIndices.forEach((cmi) =>
            connectors.push({
              path: `M ${RP_pos[ri - 1][cmi].x + cardW / 2} ${midY} V ${nextTopY}`,
              isWinnerPath: anyWinner,
            }),
          );
        } else {
          ...
          connectors.push({
            path: `M ${fromX} ${roundBottomY} V ${midY} H ${toX} V ${nextTopY}`,
            isWinnerPath: anyWinner,
          });
        }
      });
...
export default function BracketViewer({
  ...
  const desktopLayout = useMemo(() => {
    if (isMobile || viewMode === "playoffs" || containerWidth === 0)
      return {
        nodes: [],
        connectors: [],
        width: 0,
    ...
    if (useMirrored)
      return computeMirroredLayout(bracketColumns, containerWidth);
  ...
  return (
    ...
    >
      ...
      {viewMode === "playoffs" && (
        <m.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-2 mb-3">
            <span className="px-2.5 py-0.5 rounded-full bg-slate-800/80 border border-slate-700/50 text-[10px] font-bold uppercase tracking-wider text-slate-400">
              {tRound("Round of 32")}
            </span>
            <span className="text-[10px] text-slate-600">
              {playoffMatchups.length} partidos
            </span>
          </div>
          <PlayoffsList
      ...
      {thirdPlace?.matchups?.length > 0 && viewMode === "bracket" && (
        <div className="mt-3">
          <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold mb-1.5 px-1 block">
            {tRound("3rd Place Final")}
          </span>
          <div
            style={{
              width: isMobile ? "50%" : `${CARD_W}px`,
              maxWidth: "180px",
            }}
          >
            {isMobile ? (
              <MobileVerticalCard
                matchup={thirdPlace.matchups[0]}
                index={0}
                cardW={170}
                onSelect={setSelectedMatchup}
              />
      ...
      <div className="sr-only" role="tree" aria-label="Llaves del torneo">
        <ul>
          {(columns || []).map((col) => (
            <li key={col.phase} role="treeitem">
              <span>{tRound(col.phase)}</span>
              <ul role="group">
                {col.matchups.map((m, i) => (
                  <li key={i} role="treeitem">
                    {m.teamA.name} {m.aggA ?? "-"} vs {m.teamB.name}{" "}
                    {m.aggB ?? "-"}
                  </li>
                ))}
              </ul>
