  ...
  return (
    <>
      ...
      >
         ...
         <div className="flex-1 min-w-0">
           <div className="flex items-center gap-2 mb-0.5">
             <span className="text-[10px] uppercase font-bold text-indigo-400 tracking-wider">Tribuna Chat</span>
             <span className={`text-[9px] ${connected ? 'text-emerald-400' : 'text-red-400 animate-pulse'}`}>
               {connected ? '● En vivo' : '● Conectando'}
             </span>
           </div>
           {lastMsg ? (
             <p className="text-xs text-white/80 truncate">
               <span className="font-bold text-white opacity-60 mr-1">{lastMsg.user.displayName}:</span>
               {lastMsg.content}
             </p>
           ) : (
             <p className="text-xs text-white/40 italic">Nadie picanteó todavía. Podés ser el primero...</p>
           )}
         </div>
      </div>
      ...
      <div className="hidden lg:flex flex-col bg-white/[0.03] rounded-2xl border border-white/5 overflow-hidden shadow-xl lg:static lg:h-[calc(100vh-180px)] lg:backdrop-blur-md">
        ...
        {/* Messages */}
        <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-white/10">
          {messages.length === 0 ? (
        ...
        <div className="p-3 bg-white/[0.02] border-t border-white/5">
          <form onSubmit={handleSend} className="flex gap-2">
            <input type="text" placeholder="Escribí un mensaje..." value={inputVal} onChange={(e) => setInputVal(e.target.value)} disabled={!connected} className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 disabled:opacity-50" />
            <button type="submit" disabled={!connected || !inputVal.trim()} className="w-10 h-10 flex items-center justify-center rounded-xl bg-indigo-500 text-white cursor-pointer hover:bg-indigo-400 transition-colors disabled:opacity-50">
              <Send size={16} />
            </button>
          </form>
        </div>
        ...
        <div className="fixed inset-0 z-[100] bg-[#0a0f1c] flex flex-col sm:hidden">
          ...
          <div className="px-4 py-3 border-b border-indigo-500/20 bg-black/40 flex items-center justify-between shrink-0 shadow-md">
            <div className="flex items-center gap-2">
              <span className="w-8 h-8 flex items-center justify-center bg-indigo-500/20 rounded-full border border-indigo-500/30">🗣️</span>
              <div>
                <h3 className="font-bold text-white text-sm">Tribuna Chat</h3>
                <span className={`text-[10px] uppercase font-bold tracking-wider ${connected ? 'text-emerald-400' : 'text-red-400 animate-pulse'}`}>
                  {connected ? '● En vivo' : '● Conectando'}
                </span>
              </div>
            </div>
            <button onClick={() => setIsOpen(false)} className="p-2 text-white/60 hover:text-white bg-white/5 rounded-full border-none cursor-pointer">
              <X size={20} />
            </button>
          </div>

          {/* Messages */}
          <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-white/10" style={{ background: 'linear-gradient(to bottom, #0a0f1c, #05080f)' }}>
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center text-white/30 space-y-2">
                <span className="text-4xl opacity-50">🗣️</span>
                <p className="text-sm">No hay mensajes todavía.<br/>¡Escribí el primero!</p>
              </div>
            ) : (
              messages.map((m, idx) => {
                const isMe = m.user.id === user.id;
          ...
          <div className="p-3 bg-black/60 border-t border-white/10 pb-6">
            <form onSubmit={handleSend} className="flex gap-2">
              <input type="text" placeholder="Escribí un mensaje..." value={inputVal} onChange={(e) => setInputVal(e.target.value)} disabled={!connected} autoFocus className="flex-1 bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 disabled:opacity-50" />
              <button type="submit" disabled={!connected || !inputVal.trim()} className="w-12 h-[46px] flex items-center justify-center rounded-xl bg-indigo-500 text-white cursor-pointer hover:bg-indigo-400 border-none transition-colors disabled:opacity-50">
                <Send size={18} />
              </button>
            </form>
          </div>
